...
const SidebarButton: React.FC<{ icon: React.ReactNode; label: string; onClick: () => void }> = ({ icon, label, onClick }) => (
  <button
    onClick={onClick}
    className="flex items-center w-full px-3 py-2 text-left text-gray-300 transition-colors bg-white/5 rounded-md hover:bg-sky-600 hover:text-white"
  >
    {icon}
...
const Sidebar: React.FC<SidebarProps> = ({ onAddNode, isCollapsed, onToggle }) => {
  return (
    <div className={`relative z-20 flex flex-col flex-shrink-0 h-full bg-black/30 backdrop-blur-2xl border-r border-white/10 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-0' : 'w-60 p-4'}`}>
      ...
      <button 
          onClick={onToggle}
          className="absolute top-1/2 -right-[1px] transform -translate-y-1/2 translate-x-full w-5 h-20 bg-neutral-800/90 hover:bg-sky-600 border border-white/10 rounded-r-lg flex items-center justify-center text-white/70 hover:text-white transition-colors"
