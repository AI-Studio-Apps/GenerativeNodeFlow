...
export const OutputDisplay: React.FC<OutputDisplayProps> = ({ content, status, errorMessage, progressMessage, idleMessage }) => {
  if (status === NodeStatus.PROCESSING) {
    return (
      <div className="flex flex-col items-center justify-center w-full h-full p-4 space-y-2 text-center rounded-lg">
        <div className="w-8 h-8 border-4 border-t-sky-400 border-gray-600 rounded-full animate-spin"></div>
  ...
  const renderMedia = (src: string, isVideo = false) => (
      <div className="relative group w-full h-full rounded-lg flex items-center justify-center bg-black/20">
          {isVideo ? (
              <video src={src} controls className="object-contain w-full h-full rounded-lg" />
          ) : (
              <img src={src} alt="Generated output" className="object-contain w-full h-full rounded-lg" />
          )}
          <button
  ...
  return <div className="text-xs text-gray-500 flex items-center justify-center w-full h-full p-4 text-center" style={{ opacity: 0.7 }}>{idleMessage || "Output will appear here."}</div>;
};