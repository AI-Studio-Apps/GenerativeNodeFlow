    ...
    return (
        ...
        >
            <div
                ...
                className="w-6 h-6 rounded-full bg-neutral-800/80 border-2 border-black/50 group-hover/node:bg-sky-500 cursor-pointer transition-all duration-300 scale-0 group-hover/node:scale-100 flex items-center justify-center"
            >
...
const NodeComponent: React.FC<NodeProps> = ({ node, isSelected, onMouseDown, onHandleMouseDown, onResizeMouseDown, updateNodeData, onEditPrompt, edgeColors }) => {
  ...
  const statusClasses = useMemo(() => {
    switch (data.status) {
      case NodeStatus.PROCESSING: return 'border-sky-500/80 ring-2 ring-sky-500/50 animate-pulse';
  ...
  const renderNodeContent = useCallback(() => {
    switch (node.type) {
      ...
      case NodeType.VIDEO_GENERATOR:
        // Use OutputDisplay to show status (Error, Processing) and content (Preview)
        return (
           <OutputDisplay 
        ...
        return (
          <div className="flex flex-col items-center justify-center h-full w-full relative">
            {data.status === NodeStatus.ERROR ? (
                 <div className="absolute inset-0 flex items-center justify-center bg-black/80 rounded-full z-10 p-2 text-center">
                    <OutputDisplay 
                        content={null} 
                        status={data.status} 
                        errorMessage={data.errorMessage} 
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-full z-10">
                    <div className="w-6 h-6 border-2 border-t-sky-400 border-gray-600 rounded-full animate-spin"></div>
                </div>
            ) : null}
            
            <button
              onClick={() => onEditPrompt(node.id)}
              className="text-sm font-semibold transition-opacity hover:opacity-80 z-0"
              style={{ color: 'var(--node-text-color, #ffffff)' }}
            >
              Edit Prompt
            </button>
